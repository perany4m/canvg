...
(function () {
	...
	function renderFromCommands(x, y, cmd, ctx) {
		...
		for (var i = 0; i < cmd.length; i++) {
			...
			if (funcName === "lineTo") {
				...
				ctx.scale(myCmd.value[0], myCmd.value[1]);
			}
			else {
				debugger;
	...
	this.canvg = function (target, target2, target3, s, cmdLog, opts) {
		...
		ctx = proxyCtx(ctx);
		svg.loadXmlDoc(ctx, svg.parseXml(s), opts.ignoreFontStrokeFill);
	...
	function build() {
		...
		// globals
		svg.init = function (ctx, ignoreFontStrokeFill) {
			svg.Definitions = {};
			svg.Styles = {};
			svg.Animations = [];
			svg.Images = [];
			...
			svg.ignoreFontStrokeFill = ignoreFontStrokeFill;
		}
		svg.init();
		...
		svg.Element.ElementBase = function (node) {
			...
			this.render = function (orgCtx) {
				...
				var ctx = new Proxy({}, {
					get: function (target, name) {
						if (firstChange && typeof orgCtx[name] === 'function') {
							orgCtx.save();
							firstChange = false;
						}
						changed = true;
		...
		svg.Element.marker = function (node) {
			...
			this.render = function (orgCtx, point, angle) {
				var firstChange = true,
					changed = false;
				...
				orgCtx.translate(point.x, point.y);
		...
		svg.loadXmlDoc = function (ctx, dom, ignoreFontStrokeFill) {
			svg.init(ctx, ignoreFontStrokeFill);
